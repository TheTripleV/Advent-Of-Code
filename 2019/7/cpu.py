...
class CPU:
    ...
    def set_logging_states( self, to_log_states ):
        self.LOG_STATES = to_log_states
    ...
    def run( self ):
        if self.LOG_STATES and not self.FINISHED and not self.PAUSED: print( '{} Running'.format( self.name ) )
        while not self.FINISHED:
            if self.PAUSED:
                if self.LOG_STATES: print( '{} Paused'.format( self.name ) )
                return
            self.step()
            time.sleep( self.slow )

        if self.LOG_STATES: print( '{} Finished'.format( self.name ) )
    ...
    def log( self ):
        ...
        if self.name is not None: print( self.name, '> ', end='' )
...
if __name__ == "__main__":
    ...
    cpus =  [ CPU( name ) for name in [ 'A', 'B', 'C', 'D', 'E' ] ]
